...
public class UnfinishedTaskFragment extends BaseFragment {
  ...
  private static final String TAG = "UnfinishedTaskFragment";
  ...
  private void getDataFromServer(final PullToRefreshLayout refreshLayout) {
    MySubscriber subscriber = new MySubscriber<TaskModel>(getActivity()) {
      @Override
      ...
      public void onError(Throwable e) {
        ...
        HouLog.d(TAG + "新任务列表onError", e.toString());
      }
    ...
    HouLog.d(TAG, "任务列表.新任务参数：" + params.toString());
    MyApplication.rxNetUtils.getTaskService().getTaskListOne(params)
