...
public class MyCollectionFragment extends BaseFragment {
  ...
  private final static String TAG = "MyCollectionFragment";
  ...
  private void getData(final PullToRefreshLayout refreshLayout) {
    Subscriber subscriber = new MySubscriber<TrainDataModel>(getActivity()) {
      ...
      public void onError(Throwable e) {
        ...
        HouLog.d(TAG, "我的培训收藏onError" + e.toString());
      }
      ...
      public void onNext(TrainDataModel trainDataModel) {
        ...
        if (trainDataModel != null) {
          ...
          if (pageNo > 1) {
            Toast.makeText(getActivity(), getInfo(R.string.no_more_message), Toast.LENGTH_LONG)
                .show();
            pageNo--;
          } else {
    ...
    HouLog.d(TAG + "我收藏的培训参数:", params.toString());
    MyApplication.rxNetUtils.getTrainService()
        .getCollectTrain(params)
