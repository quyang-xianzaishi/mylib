...
import com.example.administrator.lubanone.bean.model.TaskModel.TaskList;
...
public class FinishedTaskFragment extends BaseFragment {
  ...
  private static final String TAG = "FinishedTaskFragment";
  ...
  private void getDataFromServer(final PullToRefreshLayout refreshLayout) {
    Subscriber subscriber = new MySubscriber<TaskModel>(getActivity()) {
      ...
      public void onError(Throwable e) {
        ...
        HouLog.d(TAG + "已完成任务列表onError", e.toString());
      }

      @Override
      public void onNext(TaskModel taskModel) {
        HouLog.d(MyApplication.isMainThread());
    ...
    params.put("type", "2");
    HouLog.d(TAG, "任务列表.已完成任务参数：" + params.toString());
